    ...
    <select
      ...
      (change)="changeMonth($event.target.value)">
        <option *ngFor="let m of months" [attr.aria-label]="i18n.getMonthFullName(m, date?.year)"
                [value]="m" [attr.selected]="m === date?.month ? true : null">{{ i18n.getMonthShortName(m, date?.year) }}</option>
    </select><select
      [disabled]="disabled"
      class="custom-select"
      ...
      (change)="changeYear($event.target.value)">
        <option *ngFor="let y of years" [value]="y" [attr.selected]="y === date?.year ? true : null">{{ i18n.getYearNumerals(y) }}</option>
    </select>
  `
})
